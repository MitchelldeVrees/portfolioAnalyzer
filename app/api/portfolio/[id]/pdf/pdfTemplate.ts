...
<body>
  ...
  <section class="page">
    ...
    <div class="block avoid-break">
      ...
      <p class="small muted" style="margin-top:8px;">Sector data is sourced from our data vendors.</p>
    </div>
  </section>
  <section class="page">
    <h2>Analysis</h2>
            ...
            ${
              negativeTiltSuggestions.length
                ? `
                  ...
                  <p class="small muted" style="margin-top:6px;">Leaders retrieved via our data vendors; final selections should consider valuation, liquidity, and your mandate.</p>
                `
                : `<p>
        ...
        <div>
          ...
          <table>
            <tbody>
              ...
              <tr><td>PEG Ratio (harmonic)</td><td class="num">${
                typeof portfolio?.advanced?.fundamentals?.weighted?.pegRatio_harmonic === "number"
                  ? portfolio.advanced.fundamentals.weighted.pegRatio_harmonic.toFixed(2)
                  : '—'
              }</td></tr>
              <tr><td>Price/Book (harmonic)</td><td class="num">${
                typeof portfolio?.advanced?.fundamentals?.weighted?.priceToBook_harmonic === "number"
                  ? portfolio.advanced.fundamentals.weighted.priceToBook_harmonic.toFixed(2)
                  : '—'
              }</td></tr>
              <tr><td>ROE (weighted)</td><td class="num">${
                typeof portfolio?.advanced?.fundamentals?.weighted?.returnOnEquity_weighted === "number"
                  ? (portfolio.advanced.fundamentals.weighted.returnOnEquity_weighted * 100).toFixed(2) + '%'
                  : '—'
              }</td></tr>
              <tr><td>Debt/Equity (weighted)</td><td class="num">${
                typeof portfolio?.advanced?.fundamentals?.weighted?.debtToEquity_weighted === "number"
                  ? portfolio.advanced.fundamentals.weighted.debtToEquity_weighted.toFixed(2)
                  : '—'
              }</td></tr>
            </tbody>
          </table>
                ...
                Array.isArray(portfolio?.advanced?.fundamentals?.rows)
                  ? portfolio.advanced.fundamentals.rows.map((r: any) => `
                    <tr>
                      ...
                      <td class="num">${typeof r.pegRatio === "number" ? r.pegRatio.toFixed(2) : '—'}</td>
                      <td class="num">${typeof r.priceToBook === "number" ? r.priceToBook.toFixed(2) : '—'}</td>
                      <td class="num">${typeof r.returnOnEquity === "number" ? (r.returnOnEquity * 100).toFixed(2) + '%' : '—'}</td>
                      <td class="num">${typeof r.debtToEquity === "number" ? r.debtToEquity.toFixed(2) : '—'}</td>
                    </tr>
                  `).join("")
                  : ""
              }
